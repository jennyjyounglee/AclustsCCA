...
TABLE1 <- summary.Aclustering(clusters.list=clusters.list,annot=annot)
TABLE1
```
...
* Fast permutation test paper: Gandy, A., & Hahn, G. (2014). MMCTest—a safe algorithm for implementing multiple Monte Carlo tests. Scandinavian Journal of Statistics, 41(4), 1083-1101. (https://onlinelibrary.wiley.com/doi/full/10.1111/sjos.12085)
...
maxB <- 300
X.groupidx <- NULL # For lasso or adaptive lasso, no need to define grouping structure
...
# Run AclustsCCA
AclustsCCA.result <- AclustsCCA(clusters.list=clusters.list,
                                X=DATA.X,
                                Y=DATA.Y,
                                Xmethod=Xmethod,
                                Ymethod=Ymethod,
...
AclustsCCA.result$permute.result # summary of permutation test
AclustsCCA.result$permute.result@num # number of permutations ran for each cluster
AclustsCCA.result$permute.result@g # number of exceedance for each cluster
```
