...
**`annot`** A preloaded annotation file that includes columns "IlmnID", "Coordinate_37", "Islands_Name", "Relation_to_Island", "UCSC_RefGene_Name".
...
**`max.dist`** Optional maximum length between neighboring variables permitting to cluster them together. Corresponds to $\bar{d}_{bp}$ in the [A-clustering  paper](https://academic.oup.com/bioinformatics/article/29/22/2884/314757?login=true) paper and the default is $1000$.

**`bp.thresh.dist`** A distance in chromosomal location. Any set of methylation sites within an interval smaller or equal to bp.dist will be potentially merged, depending on the similarity between sites at the ends of the interval. Corresponds to $\underline{d}_{bp}$ in the [A-clustering  paper](https://academic.oup.com/bioinformatics/article/29/22/2884/314757?login=true) and the default is $999$.
